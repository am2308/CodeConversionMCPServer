...
deploy_application() {
    ...
    if ! sam validate --region "$AWS_REGION"; then
    ...
    # Clean up any problematic directories COMPLETELY
    echo -e "${YELLOW}🧹 Cleaning up ALL build artifacts and backup directories...${NC}"
    rm -rf .aws-sam/ code-backup/ code-temp-backup/ code/ venv/ .venv/
    
    # Ensure .samignore is comprehensive
    cat > .samignore << 'EOF'
...
# Documentation and scripts not needed for Lambda
*.md
deploy*.sh
git-push.sh
cost-estimate.sh
...
EOF
    ...
    echo -e "${GREEN}✅ Build environment cleaned and .samignore updated${NC}"

    # Step 1: Build the SAM application in a clean environment
    echo -e "${YELLOW}🔨 Building SAM application...${NC}"
    if ! sam build --use-container --region "$AWS_REGION"; then
        echo -e "${RED}❌ SAM build failed${NC}"
        echo -e "${YELLOW}💡 Build failed. Check the error above for details.${NC}"
        exit 1
    fi
...
show_help() {
    ...
    echo -e "  • API Gateway with X-Ray tracing"
